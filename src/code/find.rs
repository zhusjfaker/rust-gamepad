...
pub fn active_gamepad(api: HidApi, gamepad_type: GamePadProduct, info: bool) -> Option<HidDevice> {
    println!(
        "There are {:#?} currently connected devices! ......",
    ...
    for device in api.device_list() {
        ...
        let device_active = match api.open(origin_vendor_id, origin_product_id) {
            Ok(active_device) => Some(active_device),
            Err(_error) => {
                if info {
                    println!(
                        "vid -> {:#?} pid->{:#?} device has error has skipped",
                        origin_vendor_id, origin_product_id
                    );
                }
                None
            }
        ...
        let manufacturer = match hid_device.get_manufacturer_string() {
            Ok(res) => res.unwrap_or("".to_string()),
            Err(_ex) => "".to_string(),
        };
